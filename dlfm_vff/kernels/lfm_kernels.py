...
if __name__ == '__main__':
    def test_GKG(nu=0.5):
        ...
        gkg = LFM_MaternKernel1d(scaled_matern_kernel)
        gkg.alpha = torch.as_tensor([0.5, 3])
        gkg.beta = torch.as_tensor([0.13, 1.])
        gkg.outputscale = torch.as_tensor([1, 1])
        gkg.lamda = torch.as_tensor([0.15, 3+1e-4]).unsqueeze(-1).unsqueeze(-1)
        print(f"===== nu: {nu}, batch shape: {gkg.batch_shape} =====")

        x1 = torch.linspace(4.14, 10.26, 5).unsqueeze(-1).repeat(2, 1, 1)
        x2 = torch.linspace(-5, 1, 4).unsqueeze(-1).repeat(2, 1, 1)
        res = gkg(x1, x2)
        print(f"x1 (shape {x1.shape}): \n{x1}")
        print(f"x2 (shape {x2.shape}): \n{x2}")
        print(f"GKG(x1, x2) (shape {res.shape}): \n{res.to_dense()})\n")
        print(f"GKG(x1, x1) diag: \n{gkg(x1, diag=True)}\n")

        print(f"===== check shape ====")
        x = torch.randn(3, 2, 5).unsqueeze(-1)  # [3, 2, 5, 1]
        res = gkg(x, x)
        print(f"GKG(x, x): x shape: {x.shape}; GKG shape {res.shape}; GKG diag shape: {gkg(x, x, diag=True).shape}\n")
        # x = torch.cat([x1, x2], dim=-2).mT.unsqueeze(-1)
        # print(f'stack x1 and x2 to get x: {x}')
        # print(f"GKG(x, x): {gkg(x, x).to_dense()}")
        # print(f"Actually GKG can accept multi-dim inputs")
